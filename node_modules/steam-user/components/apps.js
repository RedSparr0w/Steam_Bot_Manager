...
/**
 * Tell Steam that you're "playing" a game.
 * @param {array} apps - AppID or string (non-Steam game names) for the game you're playing. Empty to play nothing.
 * @param {boolean} [force=false] If true, kick any other sessions logged into this account and playing games from Steam
 */
SteamUser.prototype.gamePlayed = function(app, force = false) {
	return this.gamesPlayed([app], force);
}
 ...
 * @param {boolean} [force=false] If true, kick any other sessions logged into this account and playing games from Steam
 */
SteamUser.prototype.gamesPlayed = function(apps, force = false) {
	apps = apps.map(app => {
		if (typeof app === 'object') {
			return app;
		}

		if (typeof app === 'string' && isNaN(app)) {
			return {
	...
	function doTheThing() {
		self._send(SteamUser.EMsg.ClientGamesPlayed, apps);
	}
};

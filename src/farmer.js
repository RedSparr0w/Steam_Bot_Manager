...
const newBot = function(bot, v) {
  ...
  v.manager.on('newOffer', function(offer) {
    ...
    if(offer.partner.getSteamID64() == admin.steamID64){
      ...
      */
    v.community.getSteamUser(offer.partner, (err, otherUser) => {
      const message = (offer.message.length > 0 ? offer.message : 'Click here to view →');
      // TODO: convert to electron code
  ...
  v.client.on('steamGuard', function(domain, callback, lastcode) {
    ...
    } else if (v.shared_secret && !lastcode) {
      callback(SteamTotp.getAuthCode(v.shared_secret));
      return;
    } else {
  ...
  v.checkMinPlaytime = function() {
    ...
    v.client.once('webSession', function(sessionID, cookies) {
      const ownedPackages = v.client.licenses.map(function(license) {
        const pkg = v.client.picsCache.packages[license.package_id].packageinfo;
      ...
      v.request(`https://steamcommunity.com/my/badges/?p=${v.g_Page}`, function(err, response, body) {
        ...
        $_('.badge_row').each(function() {
          ...
          if (!v.client.picsCache.apps[appid]) {
          ...
          ownedPackages.filter(function(pkg) {
            ...
            const timeCreatedAgo = Math.floor(Date.now() / 1000) - pkg.time_created;
  ...
  v.checkCardApps = function() {
    ...
    v.client.once('webSession', function(sessionID, cookies) {
      ...
      v.request(`https://steamcommunity.com/my/badges/?p=${v.g_Page}`, function(err, response, body) {
        ...
        const infolines = $_('.progress_info_bold');

        for (let i = 0; i < infolines.length; i++) {
          const match = $_(infolines[i]).text().match(/(\d+) card drops? remaining/);

          const href = $_(infolines[i]).closest('.badge_row').find('.badge_title_playgame a').attr('href');
          if (!href) {
            continue;
          }
          ...
          if (!appLaunched) {
            ...
            let title = $_(infolines[i]).closest('.badge_row').find('.badge_title');
  ...
  v.client.on('friendMessage', function(steamID, message) {
    if (/^[\d\w]{2,5}(\-[\d\w]{4,5}){2,4}$/.test(message)) {
      v.client.redeemKey(message, function(result, details, packages) {
        games = '';
        packages.forEach(function(appID, appName) {
          games += (`${appName}\n`);
        });
        if (result == 1) {
      ...
      v.community.getSteamUser(steamID, (err, otherUser) => {
